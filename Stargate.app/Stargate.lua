...
if not component.isAvailable("stargate") then
  GUI.error("Ce programme nécessite la Porte des étoiles du Mod \"SGCraft\"", {title = {color = 0xFF8888, text = "Erreur"}})
  return
else
...
local function drawSG()
  ...
  buttons.addContactButton = GUI.framedButton(x, y, buttonWidth, 3, lineColor, lineColor, pressColor, pressColor, "Ajouter"):draw(); y = y + 3
  buttons.removeContactButton = GUI.framedButton(x, y, buttonWidth, 3, lineColor, lineColor, pressColor, pressColor, "Effacer", sizeOfContacts <= 0):draw(); y = y + 3
  buttons.connectToContactButton = GUI.framedButton(x, y, buttonWidth, 3, lineColor, lineColor, pressColor, pressColor, "Connexion", sizeOfContacts <= 0 or stargateState == "Connected"):draw(); y = y + 3
  y = y + 1
  centerText(y, lineColor, "Coûts activation"); y = y + 2
  ...
  y = y + 2
  buttons.quitButton = GUI.framedButton(x, y, buttonWidth, 3, lineColor, lineColor, pressColor, pressColor, "Quittez"):draw(); y = y + 3
...
local function connectSG(address)
  ...
  if success then
    ...
    GUI.error(reason, {title = {color = 0xFFDB40, text = "Erreur de connexion à la porte"}, backgroundColor = 0x262626})
  end
end
...
while true do
  ...
  if e[1] == "touch" then
    for _, button in pairs(buttons) do
      if button:isClicked(e[3], e[4]) then
        ...
        if button.text == "Connexion directe" then
          local data = ecs.universalWindow("auto", "auto", 36, 0x262626, true,
            {"EmptyLine"},
            {"CenterText", ecs.colors.orange, "Connexion directe"},
            {"EmptyLine"},
            {"Input", 0xFFFFFF, ecs.colors.orange, contacts.lastAddress or "Entrez l'adresse"},
            {"EmptyLine"},
            {"Button", {ecs.colors.orange, 0xffffff, "OK"}, {0x999999, 0xffffff, "Annulation"}}
          )
          if data[2] == "OK" then
            connectSG(data[1])
          end
        ...
        elseif button.text == "Ajouter" then
          ...
          local data = ecs.universalWindow("auto", "auto", 36, 0x262626, true,
            {"EmptyLine"},
            {"CenterText", ecs.colors.orange, "Ajouter un contact"},
            {"EmptyLine"},
            {"Input", 0xFFFFFF, ecs.colors.orange, "Nom"},
            {"Input", 0xFFFFFF, ecs.colors.orange, contacts.lastAddress or component.stargate.remoteAddress() or "Stargate Adresse"},
            {"EmptyLine"},
            -- remoteAddress ~= "" and
            -- {"Button", {ecs.colors.orange, 0xffffff, "OK"}, {0x999999, 0xffffff, "Добавить текущий"}, {0x777777, 0xffffff, "Отмена"}}
            -- or 
            {"Button", {ecs.colors.orange, 0xffffff, "OK"}, {0x777777, 0xffffff, "Annulation"}}
          )
          if data[3] == "OK" then
          ...
          drawAll()
        elseif button.text == "Connexion" or button.text == "Effacer" then
          local isConnect = (button.text == "Connexion")
          local names = {}; for name in pairs(contacts.addresses) do table.insert(names, name) end
          local data = ecs.universalWindow("auto", "auto", 36, 0x262626, true,
            {"EmptyLine"},
            {"CenterText", ecs.colors.orange, isConnect and "Connexion" or "Supprimer contact"},
            {"EmptyLine"},
        ...
        elseif button.text == "Déconnexion" then
          ...
          stargate.disconnect()
        elseif button.text == "Quittez" then
          buffer.clear(0x262626)
          return
        elseif button.text == "Message" then
          local data = ecs.universalWindow("auto", "auto", 36, 0x262626, true,
            {"EmptyLine"},
            {"CenterText", ecs.colors.orange, "Message"},
            {"EmptyLine"},
            {"Input", 0xFFFFFF, ecs.colors.orange, "Texte Message"},
            {"EmptyLine"},
            {"Button", {ecs.colors.orange, 0xffffff, "OK"}, {0x999999, 0xffffff, "Annulation"}}
          )
          if data[2] == "OK" then
            component.stargate.sendMessage(data[1] or "Message Vide")
          end
        end
      end
    end
  ...
  elseif e[1] == "sgMessageReceived" then
    GUI.error(tostring(e[3]), {title = {color = 0xFFDB40, text = "Envoyé par la Porte"}, backgroundColor = 0x262626})
  end
end
